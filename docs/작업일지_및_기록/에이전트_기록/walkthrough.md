...
- **편차**: 두 건이 **원인 미확정**이다. "없음"으로 덮지 않는다.
  ...
  2. **wt111 클립보드 토스트 미표시** — dev↔배포 차이라 코드 로직보다 환경 요인(HTTPS·권한·
     `navigator.clipboard` 폴백 경로)일 수 있으나 **진단 전이다.** TS 승격은 원인이 잡힌 뒤에 한다
     (`트러블슈팅/00_INDEX` §1.2 — 원인 칸을 못 채우면 색인이 아니라 소음이 된다).
- **다음 에이전트가 알아야 할 것**:
  - 🔵 **앞으로 실기동 검증은 사람이 직접 한다**(2026-09-03 사람 지시, 크레딧 절약).
    에이전트는 `tsc`·`build`까지 하고 **"실기동 검증 대기"** 로 넘긴 뒤 멈춘다 — dev 서버를 띄우지 않는다.
  - **클립보드 토스트 수정은 사람이 Sonnet에게 직접 요청**할 예정이다. Opus는 손대지 않는다.
  - wt112(삭제 기능)는 이번 검증 범위 **밖**이다 — 여전히 실기동 대기.
