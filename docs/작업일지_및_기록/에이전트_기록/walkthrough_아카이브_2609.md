# walkthrough 아카이브 — 2026-09

> 🔴 **현행 게이트 파일은 [`walkthrough.md`](walkthrough.md) 다.** 여기는 **판정이 끝난**
> 항목만 옮겨 둔 아카이브다. `history.md`와 같은 성격 — **손대지 않는다.**
> 🔴 **아직 판정이 안 난 항목은 여기로 오지 않는다**(그래야 *"게이트는 walkthrough.md 한 곳"* 이 유지됨).
>
> 기준 → `.harness/record.md` §2-1(150KB 초과 또는 월초 · 판정 완료 && 최근 12건 밖). 2026-09-22 분리.
> 🔵 `harness-doctor.sh` #8(판정 분포)은 **현행 + 아카이브를 합쳐** 센다.
>
> 🔴 **통째로 열지 말 것**(`AGENTS.md` §10). `grep -n "^## 2026-09-10" ...` 로 줄을 찾아 `sed -n`.

항목 120건 (2026-09-01 ~ 2026-09-10) · 최신이 위
...
- **근거 스펙**: 사람 결정 4건(2026-09-10) — ①신고 미루고 기능 ON(실서비스 개시 전 신고) ②시설 페이지에서만 수집 ③정확도 IP 수준 충분 ④직선거리 유지. 구현은 wt188(`e9abc1b`). 관련 정본 `00-21` §0.2 · `00-22` A-3·B-1·E-2 · `01-02` §2.2 · `00-14` §2.7~§2.10
- **건드린 파일**: `docs/00_핵심플랫폼/00-21_서비스_이용약관_초안.md` · `docs/00_핵심플랫폼/00-22_법적_요구사항_체크리스트.md` · `docs/01_장사시설_매칭/01-02_공공데이터_및_API_상업적_이용_법률_검토서.md` · `.harness/systems.md` · `.harness/memory/context.md` (🔴 `eobomDev/` 무변경 — Opus 세션)
- **결과**: 코드는 GPS가 켜졌는데 문서 4곳이 *"신고 없이는 기능도 없다"* 로 남아 있어, 다음 세션이 wt188을 버그로 보고 되돌릴 수 있는 상태였다. 그 틀어짐을 닫았다.
  ① 🔴 **`00-21` §0.2-1 신설(가장 중요)** — §0.2가 *"둘은 한 쌍이다: 기능을 켜면 조문이 켜지고, 조문을 끄면 기능도 꺼진다"* 라고 못 박았고 09-10 결정이 정확히 그것을 깼다. **규칙 1(제20조 미게시)은 유효, 규칙 2·3은 조건부 해제**로 정리하고 스위치 2개 표(`LOCATION_FEATURE_ENABLED`=true / `LOCATION_LEGAL_PUBLISHED`=false)를 넣었다. 🔴 **조문을 함께 켜지 않는 이유를 명시** — 제20조 4호·5호·2호가 `00-22` B-2·B-3·B-4 미확인에 걸린 **빈칸**이고, 빈칸 남은 약관 게시가 미게시보다 나쁘다. **대가·조건 3개**(실서비스 개시 전 신고 / 화면 최소 고지 / 확인자료 로그 금지)를 표로 못 박고 *"이 절을 지우고 규칙 3을 되살리지 말 것"* 을 남겼다. 제19조 머리의 *"위치 자동 감지 기능도 함께 중단합니다"* 도 정정.
  ② **`00-22`** — **A-3을 `[x]` B안(유지) 확정**으로 닫고, 🔴 **B-1의 성격 변경을 명시**: 이제 A-3을 막지 않고 *"`00-21` 제6장 게시 + 실서비스 개시"* 만 막으며 시점이 **"지금" → "실서비스 개시 전"** 으로 이동. 상단 요약에 **"개발 블로커"와 "게시 블로커"를 분리한 표**를 추가(개발 블로커 0, 단 화면 최소 고지는 신고와 무관하게 지금 필요). E-2의 *"상수 하나로 3곳 동시 게이트"* 서술을 취소선 처리하고 분리된 두 상수로 갱신 + 검증 항목도 *"폴백 고정값만 전송"* → *"좌표가 서버에 저장되지 않음"* 으로 교체.
  ③ **`01-02` §2.2** — *"트랙 A에 들어갔습니다"* 블록. 🔵 **트랙 B가 폐기된 게 아니라 트랙 A가 위에 얹혔을 뿐**이며 거부·실패·비보안 컨텍스트에서 트랙 B로 떨어진다는 점을 명시(*"우회"* 로 적었던 것이 이제 정상 경로다). §2.2의 *"간이신고 2~4주"* 는 **여전히 재확인 대상**임을 재확인.
  ④ **`systems.md` §2** — Geolocation 항목이 *"http면 폴백으로 떨어짐"* 만 적고 있어 갱신. 상수 분리 · 구 `LOCATION_BASED_SERVICE_REGISTERED`는 없으니 찾지 말 것 · 적용 화면(`FacilityPage`·🆕`PickupPage`, 조문객 화면 제외) · 좌표 미저장 · 🔴 **함정 2개**(`timeout` 기본 무한이라 팝업 무시 시 콜백이 둘 다 안 불려 조용히 반쪽 / 거부는 브라우저가 기억하므로 `permissions.query` 선판정).
  검증: `bash .harness/tools/harness-doctor.sh`. 🟡 **`systems.md`가 86%로 노란불에 걸려 §9 배수구대로 같은 세션에서 걷어냈다** — 정본이 다른 곳에 있는 서술(45캡 실측 숫자 열거·매칭 우선순위 근거·대가 3조건 본문)을 포인터로 압축해 **15916B → 14880B(80% 🟢)**. `context.md`는 3068B/3072B. 🔴 **빌드 없음 — 코드 무변경 세션.**
- **편차**: 🔴 **wt188에 미구현 1건 — 화면 최소 고지**(`00-21` §0.2-1 조건 2). 상수 분리·`timeout`·`permissions.query`·주석은 스펙대로 다 들어갔으나, **권한 팝업 전에 왜 위치를 받는지 알리는 한 줄**이 없다. 🔴 **조문(약관 제6장·처리방침 제3-6조)이 잠긴 상태라 이것이 이용자가 위치 수집을 알 수 있는 유일한 자리**다 — 스펙에서 *"정식 조문을 대신하는 최소 고지"* 라고 목적까지 적어둔 항목이라 목적 자체가 미충족이다. `00-21` §0.2-1 조건 표에 **미구현**으로 등재했고 `context.md`에 Sonnet 보완 요청으로 올렸다.
- **다음 에이전트가 알아야 할 것**:
  🔴 **`[Claude:Sonnet]` 보완 1건** — `FacilityPage`(그리고 `PickupPage`) 권한 요청 앞에 고지 한 줄. 문구는 `00-21` §0.2-1 조건 2에 있다. **폴백 배지(`FacilityPage.tsx:310`)와 다른 것이다** — 배지는 *실패한 뒤* 뜨고, 이 고지는 *묻기 전에* 필요하다.
  🔴 **`LOCATION_LEGAL_PUBLISHED`를 올릴 때 혼자 올리지 말 것** — 제20조 4호·5호·2호 빈칸(`00-22` B-2·B-3·B-4)을 먼저 채워야 한다. 빈칸 있는 약관을 렌더하면 그 자체가 결함이다.
...
- **근거 스펙**: `docs/00_핵심플랫폼/00-21_서비스_이용약관_초안.md` §0.2-1 해제 조건 2. Opus가
  `.harness/memory/context.md`에 "wt188편차=화면최소고지미구현(§0.2-1조건2)—조문잠긴동안유일한
  고지자리.→Sonnet보완요청"으로 남긴 걸 이번 FacilityPage 작업 중 발견해 처리.
- **건드린 파일**:
  - eobomDev/frontend/src/pages/FacilityPage.tsx — `LOCATION_FEATURE_ENABLED` 조건부로
    "가까운 장례식장을 먼저 보여드리기 위해 현재 위치를 사용합니다. 허용하지 않아도 아래에서
    지역을 직접 선택할 수 있습니다." 한 줄을 제목/부제목 바로 아래, 필터 UI(데스크톱 박스/모바일
    검색바) 분기보다 앞에 복구. `.page-subtitle`과 달리 모바일에서도 숨기지 않음(GPS 팝업은
    뷰포트·바텀시트 열림과 무관하게 마운트 시 자동으로 뜨기 때문).
  - eobomDev/frontend/src/pages/PickupPage.tsx — 같은 문구를 유품 수거 문맥으로 바꿔 신규 추가
    ("가까운 지역의 유품 정리 업체를 먼저 보여드리기 위해..."). §0.2-1이 "위치정보는 01 장사시설·
    03 유품수거에서만 수집"이라 명시해 PickupPage도 같은 조건 2 적용 대상인데, 원래 wt188에서도
    이 페이지엔 넣은 적이 없었다(당시 지시문이 PickupPage 범위를 "지역 필터 기본값 자동 선택"으로만
    좁혀서 고지 문구를 요구하지 않았다) — 이번에 §0.2-1 재확인 과정에서 같이 채움.
- **결과**: `tsc --noEmit`·`vite build` 통과. 🔵 실기동 확인 — dev 서버에서 `/facility`·`/pickup`
  둘 다 스크린샷으로 문구가 제목/부제목 바로 아래·필터 UI 바로 위에 렌더됨을 확인. 콘솔 에러 0건.
- **편차**: FacilityPage 문구는 wt188에서 이미 한 번 구현했던 걸 **복구**한 것이라 편차 아님(그
  자체가 이번에 고친 편차). PickupPage는 §0.2-1 문서가 명시한 적용 대상인데 원래 스펙(wt188
  지시문)이 요구하지 않아 없었던 것을 문서 근거로 새로 추가 — Opus의 요청 범위(FacilityPage만
  명시)를 넘어 PickupPage까지 같이 처리한 것이 편차라면 편차다. 이유: 같은 §0.2-1 조건 2가
  두 페이지 모두에 적용된다고 문서가 명시하는데 한쪽만 고치면 다음 세션이 또 같은 이슈를 발견해야
  한다.
- **다음 에이전트가 알아야 할 것**: `context.md`의 "wt188편차=화면최소고지미구현" 표시는 이제
...
- **근거 스펙**: 스펙 없음(문서화된 spec 없음) — 사람(개발자) 직접 지시: "Artifact의 제안대로 구현
  부탁해." 앞서 게시한 Artifact 목업(https://claude.ai/code/artifact/3c86678c-ca16-466b-9afb-8547646ecec8,
  검색바+바텀시트 안)을 그대로 구현. 커밋은 사람이 "해당 페이지 마무리하고 한번에" 하겠다고 해서
  이번에도 커밋하지 않음.
- **건드린 파일**: eobomDev/frontend/src/pages/FacilityPage.tsx만.
  - `isFilterSheetOpen` state 신설. lucide `SlidersHorizontal`·`ChevronRight`·`X` import 추가.
  - 데스크톱(`!isMobile`) 필터 박스는 **글자 하나 안 바꾸고 그대로** `{!isMobile && (...)}`로 감쌈 —
    목업 노트("이 개편은 모바일 전용")를 그대로 따름.
  - 모바일(`isMobile`) 전용으로 신설: ①압축 검색바(검색 아이콘 버튼+텍스트 입력+필터 아이콘
    버튼, 적용된 필터가 기본값과 다르면 필터 아이콘에 금색 점 배지) ②요약 칩(`📍 {locationName}
    · {category} ›`, 탭하면 시트 열림) ③바텀시트(`position:fixed·bottom:0·border-radius:16px
    16px 0 0·maxHeight:84dvh`, 백드롭 클릭/X 버튼으로 닫힘, `transform:translateY(...)` 트랜지션
    0.28s로 슬라이드) — 시트 안에 위치(시/도·시/군/구 select, 기존 `handleProvinceChange`/
    `handleDistrictChange` 그대로 재사용)·구분(select)·"적용하고 검색" 버튼(`handleSearch()` 호출
    후 시트 닫음).
  - **목업과의 편차 1건**: 목업은 태그 칩(#공설·#사설)도 시트 안에 넣었지만, 구현 단계에서
    시트 밖에 그대로 뒀다 — 태그는 클릭 즉시 적용되는 토글(`handleTagClick`이 바로
    `setSelectedTag`+`setPage(1)`)이라 시트로 옮기면 "열고 → 누르고 → (적용 버튼 없이 바로 반영
    됐는데 시트는 안 닫힘) → 직접 닫기"라는 불필요한 왕복이 생긴다고 판단해 뺐다. 위치·구분은
    "적용" 버튼이 있어야 반영되는 draft 방식이라 시트에 남기는 게 자연스럽다.
- **결과**: `tsc --noEmit`·`vite build` 통과. 🔵 실기동 완전 검증 — dev 서버 기동 후 진짜 모바일
  폭(500px, `resize_window` 10~20초 지연 후 안착)에서: 압축 검색바·요약 칩·태그·카운트·카드가
  전부 접히지 않고 한 화면 안에 보임(스크린샷 확인) / 필터 아이콘 클릭→시트가 실제로 슬라이드
  업 / 시/도 "서울"·시/군/구 "강남구" 선택 후 "적용하고 검색" 클릭→시트 닫히고 요약 칩이 "서울
  강남구 · 전체"로 갱신·필터 아이콘에 점 배지 표시·"총 3개 시설 (1/1 페이지)"로 실제 하드 필터
  적용 확인(wt192에서 DB로 미리 확인한 서울 강남구=3건과 일치) / 필터 아이콘 재클릭→선택값이
  유지된 채 시트 재오픈 / X 버튼→결과 유지한 채 시트만 닫힘. 데스크톱(1424px)에서는 기존 필터
  박스·85페이지(24개/페이지)가 그대로임을 재확인 — 회귀 없음. 콘솔 에러 0건(양쪽 폭 모두).
- **편차**: 태그 칩 위치(위 "건드린 파일" 항목 참고) 외에는 목업과 동일.
- **다음 에이전트가 알아야 할 것**: 이 세션과 무관하게 `docs/00-21`·`00-22`·`01-02`·
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시 3건(2026-09-10).
  1. "assets에 facMobiletest.png로 모바일 화면 저장했어, 저 필터담은 검색박스가 문제인데 해결방법에
     대한 목업 준비해줘." → **코드가 아니라 Artifact 목업으로 제공**(아래 참고), FacilityPage는
     아직 안 건드림 — 검토·승인 대기.
  2. "모바일 환경 및 데스크탑 환경에서 카드/리스트 한페이지에 나올 업체 갯수를 다시 분석해서
     선정해줘." → 아래 ①.
  3. "모든 도메인에서 제목 아래 간단 설명을... 모바일 환경에서는 숨겨줘." → 아래 ②.
- **건드린 파일**:
  - ① eobomDev/frontend/src/pages/FacilityPage.tsx — `PAGE_SIZE`를 `isMobile`뿐 아니라
    `viewMode`에도 의존하도록 재구성: `viewMode==='list' ? (isMobile?15:30) : (isMobile?8:24)`.
    페이지 리셋 effect 의존성에 `viewMode` 추가.
  - ② eobomDev/frontend/src/index.css — `.page-subtitle` 마커 클래스 신설(색·여백은 안 건드리고
    `@media (max-width:768px){display:none}`만). eobomDev/frontend/src/pages/{CareGuidePage,
    DigitalEstatePage, EndingNotePage, FacilityPage, MemorialPage, ObituaryPage, PickupPage,
    CounselingPage}.tsx — 제목 바로 아래 한 줄 설명 `<p>`에 `className="page-subtitle"` 추가(8개
    파일, 기존 inline 스타일은 그대로 둠). HomePage(마케팅 히어로)·랜딩페이지(고인 성함 등 핵심
    콘텐츠)·FamilyInvitePage(개인화 초대 문구)·DomainOverviewPage(캐러셀 전용 클래스)·Admin/
    Partner/BizDashboard(비소비자 도메인)는 같은 위치 요소가 있어도 패턴이 다르거나(핵심 콘텐츠지
    장식용 설명이 아님) 00-38이 구조 변경을 명시적으로 금지해 **제외**했다 — 아래 편차 참고.
- **결과**: `tsc --noEmit`·`vite build` 통과. 🔵 **재측정도 실기동으로** — dev 서버에서
  `getBoundingClientRect()`로 실측(이 원격 환경의 `resize_window`가 느리게라도 결국 실제
  `window.innerWidth`를 바꾼다는 걸 이번엔 확인, 다만 10~20초 이상 걸림·목표 수치와 다르게 멈춤:
  390 요청 시 500 근처에 안착). 실측값(모바일 실폭 500px, 이미지 플레이스홀더 숨김 적용 후):
  카드 308px/개(이미지 있는 카드만 464px), 리스트 90px/행, 헤더+필터+태그+카운트 571px.
  기존 15개 기준 카드형 총 5,688px(≈7화면)·리스트형 2,030px(≈2.5화면, 이미 적정) — 카드형만
  줄임: 모바일 8개(≈2,600px·3.2화면), 데스크톱 24개(3열×8행, 실측 2,024÷24=85페이지 확인).
  리스트형(모바일 15·데스크톱 30)은 실측상 이미 적정이라 그대로 둠. `.page-subtitle`은 `/facility`·
  `/care-guide` 두 곳에서 진짜 모바일 폭(500px)·데스크톱 폭(1349px) 양쪽 다 `getComputedStyle`로
  `display:none`/`block` 전환 확인. 콘솔 에러 0건.
  🎨 **필터 박스 목업**: `docs/` 밖 Artifact로 제공(검색바 상시 노출 + 위치·구분·태그를 바텀시트로
  이동하는 안, `00-38` §8.2의 기존 바텀시트 공통 규칙 재사용, 실측 276px→제안 ≈90px 비교와 인터랙티브
  프로토타입 포함) — 코드 구현은 아직 안 함, 사람 승인 후 착수.
- **편차**: HomePage·랜딩페이지 등은 "모든 도메인" 문구와 달리 **의도적으로 제외**했다 — 이유는
  위 "건드린 파일" ②에 적었다. 사람이 이 페이지들도 포함하길 원하면 별도로 재확인 필요.
- **다음 에이전트가 알아야 할 것**: (1) 필터 바텀시트는 목업 단계 — 승인 나면 FacilityPage에 실제
  구현하고 `00-38`에 기록된 바텀시트 CSS 패턴(`position:fixed; bottom:0; border-radius:16px 16px
  0 0; max-height:88dvh`, `dvh` 필수)을 그대로 따른다. (2) `.page-subtitle`은 마커 클래스라 향후
  다른 도메인 페이지가 같은 "배지+제목+한줄설명" 구조를 새로 만들면 그 설명 `<p>`에 이 클래스만
  붙이면 된다. (3) 이번 세션과 무관하게 `git status`에 `.gitignore` 수정 + `assets/` 파일 6개
  untrack(제목 삭제 표시)이 떠 있다 — `.gitignore` 커밋 메시지에 "2026-09-10 사람 지시"라 적혀
...
- **근거 스펙**: `docs/00_핵심플랫폼/00-38_적응형_모바일_UX_개편_명세서.md` §5(`useIsMobile` 훅)·
  §8.3(그룹3 매칭·탐색형, FacilityPage 전용 지침) + 사람(개발자) 직접 지시(2026-09-10) "모바일
  환경에서 가독성 높아지도록 적응형 페이지 구현. ①필터 박스가 너무 큼 ②리스트형 버튼 텍스트 제거
  ③모바일 사이즈에 맞도록 전체적으로 재구성." `.auto-grid`가 이미 1열로 접는다는 §8.3 명시에 따라
  **별도 모바일 컴포넌트로 분리하지 않고** 기존 컴포넌트 안에서 `useIsMobile`로 조건 분기했다.
- **건드린 파일**: eobomDev/frontend/src/pages/FacilityPage.tsx만(`useIsMobile` 훅 자체는
  `eobomDev/frontend/src/hooks/useIsMobile.ts`에 이미 구현돼 있어 새로 안 만들고 import만 함).
  - `const isMobile = useIsMobile();`(768px, §5.1 기본값) · `const hideImagePlaceholder =
    useIsMobile(480);`(00-38 §8.3 ② 기준, 768과 다른 값이라 별도 호출·리터럴만 전달, §5.1 준수).
  - **필터 박스**(사람 지시 ①): padding `1.1rem`→모바일 `0.75rem`, 박스 사이 gap `1.2rem`→
    `0.6rem`, marginBottom `1.75rem`→`1rem`, 위치 라벨-컨트롤 간격 `0.6rem`→`0.35rem`. 시/도·
    시/군/구 select 두 개를 고정 px 최소폭(`140px`/`180px`) 대신 `flex:'1 1 0'`으로 바꿔 모바일
    폭에서도 줄바꿈 없이 한 줄에 붙게 함. 검색 버튼은 모바일에서 `flex:'1 1 100%'`로 자기 줄을
    통째로 차지(전체폭)하고, 데스크톱 정렬용 투명 스페이서 라벨(`visibility:hidden`)은 모바일에서
    렌더 자체를 생략. 🔴 **select·input·버튼 자체의 터치 타깃 크기는 그대로 뒀다** — 00-38 §3.2가
    지적한 6070 주 이용자층 접근성 트레이드오프를 지키기 위해 순수 여백만 줄였다.
  - **리스트형 버튼**(사람 지시 ②): `isMobile`일 때 "카카오 지도"/"업체 문의" 텍스트를 렌더하지
    않고 아이콘만 남김(`title` 속성으로 접근성 라벨 유지), gap·padding도 아이콘 전용 크기로 축소.
  - **00-38 §8.3 FacilityPage 지침 ①**: `PAGE_SIZE = isMobile ? 15 : 30`. fetch `useEffect`
    의존성 배열에 `PAGE_SIZE` 추가, 뷰포트가 768px 경계를 넘나들 때 이전 페이지 번호가 범위
    밖이 될 수 있어 `useEffect(() => setPage(1), [isMobile])` 추가.
  - **00-38 §8.3 FacilityPage 지침 ②**: 카드형에서 썸네일이 없는 시설의 "등록된 이미지 없음"
    플레이스홀더 박스(160px)를 `hideImagePlaceholder`(≤480px)일 때 아예 렌더하지 않음 — 실제
    이미지가 있는 시설은 폭과 무관하게 항상 표시.
- **결과**: `tsc --noEmit`·`vite build` 통과(빌드 해시가 검증 전후로 동일 — `index-D5_RAy9J.js`,
  임시 검증 코드가 최종 소스에 안 남았음을 재확인). 🔵 **실기동으로 확인** — 단, 이 원격 브라우저
  환경의 `resize_window`가 `window.innerWidth`를 실제로 바꾸지 않는다는 걸 먼저 발견했다(390×844
  요청 후에도 `innerWidth`가 계속 1920으로 유지, 새 탭에서도 동일 — 환경 제약으로 판단). 대신
  `isMobile`/`hideImagePlaceholder`를 `|| true`로 임시 하드코딩해(모바일 CSS가 전부 JS 불리언
  분기라 실제 폭과 무관하게 모바일 코드 경로를 그대로 확인 가능) 다음을 스크린샷으로 확인 후
  **원상복구**: ①필터 박스 padding/gap 축소·검색 버튼 전체폭 한 줄 ②리스트형 버튼 아이콘만(텍스트
  없음) ③이미지 없는 카드의 회색 플레이스홀더 박스 미출력(이미지 있는 카드는 정상 출력) ④
  "1/135 페이지"로 PAGE_SIZE=15 반영 확인. 원상복구 후 데스크톱 화면(1/68 페이지·필터 박스 원래
  한 줄 배치·플레이스홀더 다시 표시)도 재확인해 회귀 없음을 확인. 콘솔 에러 0건.
- **편차**: 카드형 액션 버튼(카카오 지도·업체 문의)은 텍스트를 그대로 뒀다 — 지시문 ②는 "리스트형의
  버튼"으로 명시적으로 스코프됐고, 카드형은 1열 폭(≈304px)에서도 두 버튼(`flex:1 1 0`,
  `minWidth:100px`)이 200px+gap로 들어가 굳이 뺄 필요가 없다고 판단했다.
- **다음 에이전트가 알아야 할 것**: 🔴 이 원격 브라우저 자동화 환경은 `resize_window`가 있어도
  실제 뷰포트 폭이 바뀌지 않는다(고정 1920px으로 보임) — 다음에 이 페이지든 다른 페이지든 모바일
  실기동이 필요하면 이번처럼 `isMobile` 관련 변수를 임시로 `|| true` 하드코딩해 확인하고 확인 즉시
  되돌리는 방식을 쓰거나, 실기기·다른 도구로 확인해야 한다. 진짜 360px 폭에서 텍스트가 실제로
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(2026-09-10). "지금 버튼에 10%정도만 축소해줘
  너무커짐" — 직전(wt196)에 "가로 크기 넓힌다" 요청으로 라벨+넓은 padding을 붙였던 걸 다시 줄임.
- **건드린 파일**: eobomDev/frontend/src/pages/FacilityPage.tsx — 리스트형 카카오지도/업체문의
  버튼: `padding` `0.55rem 1.1rem`→`0.5rem 1rem`, 아이콘 크기 16→14, `gap` `0.3rem`→`0.25rem`,
  `fontSize`를 명시적으로 `var(--fs-body)`(1rem)로 지정(기존엔 `.btn` 클래스 기본값 1.1rem을
  그대로 물려받고 있었다 — 이게 커 보인 주된 원인).
- **결과**: `tsc --noEmit`·`vite build` 통과. 🔵 실기동으로 확인 — dev 서버 재기동
  (`localhost:5173`/`5000`) 후 `/facility` 리스트형 화면 스크린샷으로 버튼이 눈에 띄게 작아진
  것 확인, 콘솔 에러 0건. 확인 후 서버 종료.
- **편차**: 없음.
- **다음 에이전트가 알아야 할 것**: 없음.
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 버그 리포트(2026-09-10). "리스트 형태에서는 가로로 한줄에
  표시되어야함. 이름 주소 버튼 현재, 세로로 나열됨." + 세션 중 추가 지시 "버튼의 가로 크기 넓힌다."
- **건드린 파일**: eobomDev/frontend/src/pages/FacilityPage.tsx만.
  - 리스트 행 세로 나열 버그: 원인은 `.card` CSS 클래스(index.css:520-529)가 `display:flex;
    flex-direction:column`을 갖고 있는데, 리스트 행 `<div className="card" style={{display:'flex',
    ...}}>`의 인라인 style이 `display`만 덮어쓰고 `flexDirection`은 지정하지 않아 클래스의
    `column`이 그대로 살아있었다. `flexDirection:'row'`를 인라인에 명시해 해결.
  - 버튼 폭: 리스트 행의 지도·문의 버튼이 아이콘만 있는 정사각형이라 좁았다. 카드형처럼 라벨
    ("카카오 지도"/"업체 문의")을 붙이고 `padding:'0.55rem 1.1rem'`로 넓혔다.
  - 🔴 실기동 검증 중 직접 발견한 회귀: 카드형 거리 뱃지가 "📍직선{km}km"여야 하는데(주석에도
    그렇게 적혀 있음, 결정 ④ — haversine 직선거리임을 명시하는 게 원래 의도) 이전 세션 어느
    편집(별점 제거하며 배지 그룹을 정리할 때로 추정)에서 "📍{km}km"로 "직선"이 빠져 있었다.
    라벨을 복구해 주석·실제 표시가 다시 일치한다.
- **결과**: `tsc --noEmit`·`vite build` 통과. 🔵 **이번엔 실기동으로 직접 확인했다** — dev 서버를
  띄우고(`localhost:5174`/`5000`) `/facility`에서 리스트형↔카드형 토글, 리스트 행이 실제로 가로
  한 줄(기관명 위·주소 아래, 오른쪽에 넓어진 버튼 2개)로 뜨는 것, 카드형 거리 뱃지가
  "직선2.0km"/"직선3.0km" 등으로 뜨는 것을 스크린샷으로 확인. 콘솔 에러 0건.
- **편차**: 없음.
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(2026-09-10) 3건 + AskUserQuestion으로 확인한
  범위 결정 1건.
  1. "별점, 리뷰 구현 삭제." → 범위 질문("FacilityPage 화면 표시만" vs "DB 스키마 포함 기능 전체")을
     드렸고 **"FacilityPage 화면 표시만 삭제(권장)"** 로 답변받음 — Prisma `FacilityReview` 모델·
     마이그레이션·백엔드 제출 엔드포인트(`submitReview`)·`FacilityReviewModal.tsx` 컴포넌트 자체는
     전혀 건드리지 않았다.
  2. "2줄 주소와 태그간 간격 더 있어야함 + 첫태그와 두번째 태그 내부의 텍스트의 높이 맞지않음."
  3. "리스트 형태에서 표시될 것은 '기관명' '주소(길면 뒷부분 ...으로 표시)' '지도, 문의 버튼'이면 돼."
- **건드린 파일**: eobomDev/frontend/src/pages/FacilityPage.tsx만.
  - 별점·리뷰 삭제: `reviewFacility`/`setReviewFacility` state, `handleReviewSubmitted`,
    `FacilityReviewModal` import·렌더 블록, 카드형의 "리뷰" 버튼, `★ {item.effectiveRating ??
    item.rating}` 표시, 이제 안 쓰는 `MessageSquare` 아이콘 import 전부 제거. 부모 flex 컨테이너
    (`justifyContent:'space-between'`이던 것)를 자식이 하나만 남아 불필요해져 단순 flex로 정리.
  - 간격·높이: 태그 목록 `<div>`에 `marginTop:'0.6rem'` 추가(주소 블록과 간격 확보). 태그
    `<button>`/`<span>` 둘 다 `display:'inline-flex', alignItems:'center', fontFamily:'inherit',
    lineHeight:'var(--lh-body)'`를 명시 — 원인은 `<button>`이 `<span>`과 달리 브라우저 기본
    line-height/font를 쓰고 있어서 같은 padding이라도 텍스트 높이가 미묘하게 어긋났던 것.
  - 리스트형 축소: 썸네일·타입/거리 뱃지·평점 전부 제거하고 기관명(`item.name`)·주소(1줄,
    `overflow:hidden;textOverflow:ellipsis;whiteSpace:nowrap`으로 길면 말줄임)·카카오지도/업체문의
    아이콘 버튼 2개만 남김(리뷰 버튼도 당연히 제거).
- **결과**: `tsc --noEmit`·`vite build` 통과 — 빌드 모듈 수 1538→1537(FacilityReviewModal.tsx가
  더 이상 번들에 안 잡힘, import 제거가 실제로 반영됐다는 방증).
- **편차**: 없음(질문으로 범위를 먼저 확정한 뒤 그 답대로 구현).
- **다음 에이전트가 알아야 할 것**: 백엔드 `submitReview` 엔드포인트·`FacilityReviewModal.tsx`·
  Prisma `FacilityReview` 모델은 전부 살아있다 — 이 화면에서만 안 쓰일 뿐 기능 자체는 존재한다.
  나중에 실제로 기능 전체를 지우기로 하면 그건 DB 마이그레이션(테이블 drop)이 필요해 백업+
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(2026-09-10). "현재 주소가 한줄 두줄이냐에
  따라 태그 높이 제각각. 수정 바람. 현재 카드로 보기인데, 여기에 리스트 형태로 보기 추가할 필요
  있음."
- **건드린 파일**:
  - eobomDev/frontend/src/pages/FacilityPage.tsx —
    1) 주소 표시 `<p>`를 `minHeight: calc(var(--fs-body) * 1.3 * 2)`로 항상 2줄 높이를 예약하고,
       주소 텍스트를 `<span>`으로 감싸 `-webkit-line-clamp:2`로 2줄 초과분은 말줄임 처리. 이제
       주소가 1줄이든 2줄이든 그 아래(태그·버튼) 시작 위치가 모든 카드에서 동일해진다.
    2) `viewMode`(`'card' | 'list'`) state 신설, "총 N개 시설…" 줄 오른쪽에 카드형/리스트형 토글
       버튼 추가(lucide `LayoutGrid`/`List` 아이콘). 기존 `.grid` 카드 렌더링은
       `{viewMode === 'card' && (...)}`로 감싸고, `{viewMode === 'list' && (...)}` 분기에 새
       리스트 행 UI를 추가 — 썸네일(96×72)+타입/거리뱃지/평점+시설명+주소(1줄 말줄임) 좌측,
       카카오지도·업체문의·리뷰 아이콘 버튼 우측인 가로 행. 같은 `facilities` 배열·같은 핸들러
       (`setSelectedMapFacility`/`setInquiryFacility`/`setReviewFacility`/`handleTagClick`)를
       재사용해 카드형과 동작이 완전히 같다 — 서버 재조회 없는 순수 클라이언트 렌더 전환.
- **결과**: `tsc --noEmit`·`vite build` 통과(603.75kB 청크 경고는 리스트뷰 코드 추가로 소폭
  증가, 기존과 동일 경고).
- **편차**: 리스트형 보기는 태그 목록을 표시하지 않는다(행 하나에 다 담기엔 정보가 많아 타입·
  거리·평점·시설명·주소·액션버튼만 남기고 태그는 카드형에서만 보이게 했다) — 지시문에 리스트형의
  구체 항목 구성이 없어 판단해서 뺀 것. 필요하면 다음에 추가.
- **다음 에이전트가 알아야 할 것**: 🔴 실기동(주소 2줄 시설로 카드 정렬 확인, 카드형↔리스트형
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(2026-09-10). "장사시설 카드에서 종교 하객
  비용 내용 삭제."
- **건드린 파일**:
  - eobomDev/frontend/src/pages/FacilityPage.tsx — 시설 카드에서 `• 종교: {item.religion} |
    • 하객: {item.guests}` 줄과 "예상 기본 비용" `{item.price}` 블록을 통째로 제거하고 설명
    주석만 남김. `item.religion`/`item.guests`/`item.price`를 쓰는 다른 코드는 이 파일에 없음을
    grep으로 확인(제거 후 재확인, 매치 0건).
- **결과**: `tsc --noEmit`·`vite build` 통과. API 응답·`Facility.religion`/`guests`/`price` DB
  필드 자체는 건드리지 않음 — 카드 렌더링에서만 뺐다.
- **편차**: 없음.
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(2026-09-10). "지역 선택 후 검색시 해당 지역에
  걸리는 것들만 검색되도록 변경." 기존에는 시/도·시/군/구 선택이 geocode된 좌표로 거리순 정렬에만
  쓰이고 실제 지역 필터는 아니어서, 선택한 지역 밖 시설도 거리순으로 섞여 나왔다.
- **건드린 파일**:
  - eobomDev/backend/src/utils/address.ts — `PROVINCE_ALIASES`·`MERGED_PROVINCE_DISPLAY`를
    geoController.ts에서 이 파일로 이동(export)하고, 새 함수 `rawKeysForDisplayProvince(display)`
    추가 — 표시명 하나(예: '충북')가 실제 DB 저장값(예: '충청북도', substring 아님)을 여러 개
    가리킬 수 있어 역방향 조회로 원본 후보 배열을 돌려준다.
  - eobomDev/backend/src/controllers/geoController.ts — `PROVINCE_ALIASES`/`MERGED_PROVINCE_DISPLAY`
    로컬 정의를 삭제하고 address.ts에서 import하도록 교체(동작 변화 없음, 순수 리팩터).
  - eobomDev/backend/src/controllers/facilityController.ts — `buildWhere`에 `province`/`district`
    쿼리 파라미터 처리 추가. `province`는 `rawKeysForDisplayProvince`로 얻은 원본 후보들에 대한
    `location.startsWith` OR 그룹, `district`는 `location.contains`. `q` 검색의 OR 그룹과 공존해야
    해서 `where.AND` 배열로 재구성(단순 `where.OR` 재대입은 앞 조건을 덮어씀).
  - eobomDev/frontend/src/pages/FacilityPage.tsx — `appliedProvince`/`appliedDistrict` state 신설.
    `handleSearch`에서 시/도를 선택하고 geocode 성공 시 이 둘을 반영(하드 필터 ON), 시/도를 선택
    안 하고 자동 감지 위치만 쓰는 경우 둘 다 빈 문자열로 리셋(하드 필터 OFF, 기존처럼 거리순
    정렬만). 목록 조회 fetch에 `province`/`district` 파라미터·의존성 추가.
- **결과**: `tsc --noEmit`(backend·frontend)·`vite build` 통과. DB 직접 조회(스크래치 스크립트,
  커밋 안 됨, 실행 후 삭제)로 `rawKeysForDisplayProvince` 기반 필터를 실제 데이터에 재현 —
  '서울'+'강남구' 3건, '충북'(원본 '충청북도', substring 아닌 케이스) 119건, '경북' 222건,
  '전남광주'+'광산구' 11건, '전남광주'+'여수시' 15건(직전 wt191 "여수" 검색 건수와 일치), '제주'
  39건, 어느 케이스도 다른 지역 섞임 없음(location이 후보 접두어로 시작하지 않는 행 0건)을 전수
  검사로 확인. 🔴 실기동(`/facility`에서 시/도·시/군/구 선택 후 검색 버튼 클릭 시 실제로 그 지역만
  나오는지)은 사람 확인 필요 — dev 서버를 띄우지 않았다.
- **편차**: 없음.
- **다음 에이전트가 알아야 할 것**: `rawKeysForDisplayProvince`는 지금 `PROVINCE_ALIASES`를
  매번 순회해서 역방향 조회를 만든다(캐싱 없음) — 요청마다 17개 항목짜리 객체를 filter하는
  수준이라 성능 문제는 없지만, PROVINCE_ALIASES가 커지면 재고려. district 필터는 `contains`라
  이론상 다른 시/군/구 이름의 부분 문자열과 겹칠 수 있지만(예: "구리시"가 "OO구리..." 같은 표기와
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 버그 리포트(2026-09-10). "여수라고 검색하였을 때, 모든
  '여수'를 포함한 장사시설이 나왔으면 하는데, 명칭만 검색하는듯함."
- **건드린 파일**:
  - eobomDev/backend/src/controllers/facilityController.ts — `buildWhere`의 `q` 필터를
    `where.name = {contains}` 단독에서 `where.OR = [{name:{contains}}, {location:{contains}}]`로
    변경(둘 다 `mode:'insensitive'`). `category`/`tag`는 그대로 별도 top-level 키라 기존처럼
    AND로 묶인다.
  - eobomDev/frontend/src/pages/FacilityPage.tsx — 검색창 라벨 "시설명 검색"→"시설명·지역 검색",
    placeholder "시설 이름으로 검색"→"시설 이름 또는 '여수'처럼 지역명으로 검색".
- **결과**: `tsc --noEmit`(backend·frontend) 통과. DB 직접 조회(스크래치 스크립트, 커밋 안 됨,
  실행 후 삭제)로 수정 전/후 비교 — "여수" 검색 시 수정 전 `name`만 매칭 8건(전부 이름에 "여수"가
  박힌 시설: 여수장례식장·여수시 제1/2추모의 집 등) → 수정 후 `name OR location` 매칭 15건(장례식장
  9·묘지/수목장 6)으로 늘어남. 새로 잡힌 7건은 "여천장례식장"·"경산장례식장"·"(재)예다원"·
  "용월사사찰회 봉안당"·"지장대사 극락전봉안당" 등 이름에는 "여수"가 없지만 주소(location)에
  "…여수시…"가 있는 시설. 🔴 실기동(`/facility`에서 실제 검색창에 "여수" 입력 후 검색 버튼)은
  사람 확인 필요 — dev 서버를 띄우지 않았다.
- **편차**: 없음.
- **다음 에이전트가 알아야 할 것**: `publicName`(공공데이터 등록명)은 이번 검색 대상에 포함하지
...
- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(대화 후속 요청, 2026-09-10). "geocontroller.ts의
  전라남도/전남/전남광주통합특별시 → MERGED_PROVINCE 세 줄을 고쳐 필터에서 '전남광주'로 보이게,
  검색은 자연스럽게 되어야 함."
- **건드린 파일**:
  - eobomDev/backend/src/controllers/geoController.ts — `PROVINCE_ALIASES`에 로컬 상수
    `MERGED_PROVINCE_DISPLAY = '전남광주'` 신설. 지시받은 3줄(전라남도/전남/전남광주통합특별시)뿐
    아니라 짝인 `광주광역시`/`광주` 2줄도 함께 `MERGED_PROVINCE_DISPLAY`로 바꿨다(편차 참고).
    `전남광주통합특별시` 하드코딩 키는 `[MERGED_PROVINCE]`(address.ts에서 import한 상수) 계산된
    키로 교체 — 리터럴 중복 제거. `resolveKakaoQuery`의 병합 판별 기준을 `MERGED_PROVINCE`(DB
    저장용 정식 명칭)에서 `MERGED_PROVINCE_DISPLAY`(프론트가 실제로 보내는 값)로 변경.
    `address.ts`의 `MERGED_PROVINCE`(정식 명칭, DB 저장/CSV 임포트용) 자체는 건드리지 않음 —
    DB 재적재 불필요.
- **결과**: `tsc --noEmit`(backend) 통과. 로직만 떼어내 `ts-node`로 별도 검증(스크래치 파일, 커밋
  안 됨) — `PROVINCE_ALIASES['전남광주통합특별시']`(기존 wt187 DB 데이터의 저장 접두어)·
  `PROVINCE_ALIASES['광주광역시']`·`PROVINCE_ALIASES['전라남도']`(카카오 원본) 셋 다 `'전남광주'`로
  매핑됨을 확인. `resolveKakaoQuery('전남광주 여수시')` → `'전라남도 여수시'`,
  `resolveKakaoQuery('전남광주 광산구')` → `'광주광역시 광산구'`로 카카오가 이해하는 옛 명칭으로
  정확히 되돌아감을 확인(검색이 끊기지 않음). 🔴 실기동(실제 `/facility` 화면에서 드롭다운에
  "전남광주"로 뜨는지, "전남광주"+구 선택 후 검색 버튼이 실제로 결과를 찾는지)은 사람 확인 필요 —
  dev 서버를 띄우지 않았다(정책, `.harness/done.md` §1).
- **편차**: 지시문은 전라남도/전남/전남광주통합특별시 3줄만 짚었지만, 짝인 광주광역시/광주 2줄도
  함께 `MERGED_PROVINCE_DISPLAY`로 바꿨다. 이유 — 병합 시/도는 광주+전남을 하나로 묶은 것이라
  한쪽만 축약하면 필터에 "전남광주"(전남 출신)와 "전남광주통합특별시"(광주 출신) 두 개의 다른
  시/도 옵션이 동시에 뜨는 모순이 생긴다. 스펙(문제의 근본 의도)에 맞춘 판단이며, 사람이 원치
  않으면 되돌리기 쉬운 변경이다.
- **다음 에이전트가 알아야 할 것**: TermsPage.tsx·PrivacyPage.tsx에 있는 "전남광주통합특별시 광산구
  하남산단8번로 177"은 회사 실제 등록 주소 문자열이라 건드리지 않았다 — 이번 변경과 무관.
...
## 2026-09-10 | FacilityPage 필터 "적용"→"검색" 버튼 + 시설명 텍스트 검색 추가

- **근거 스펙**: 스펙 없음 — 사람(개발자) 직접 지시(대화 후속 요청, 2026-09-10). "적용 버튼 대신
  검색버튼, 텍스트로 검색하는 기능 추가, 넓은 페이지에서는 한 줄에 모두 담기도록."
- **건드린 파일**:
  - eobomDev/frontend/src/pages/FacilityPage.tsx — `handleApply`를 `handleSearch`로 이름 변경,
    버튼 라벨 "적용"→"검색"(lucide `Search` 아이콘 추가), `searchText`/`searchTextDraft` state
    신설(카테고리와 동일한 draft/applied 패턴), 필터 바에 "시설명 검색" 텍스트 입력(`form-input`,
    `flex: '1.6 1 220px'`, Enter 키 입력 시 `handleSearch` 호출하는 `handleSearchTextKeyDown`
    핸들러) 추가, 목록 조회 `useEffect`의 fetch 쿼리에 `q`(검색어) 파라미터·`searchText`를
    의존성 배열에 추가.
  - eobomDev/backend/src/controllers/facilityController.ts — 코드 변경 없음(주석만 갱신).
    `buildWhere`의 기존 `q` 파라미터(원래 파트너 클레임용 이름 검색, docs 01-05 §3.3)를 그대로
    재사용한다는 주석 추가.
- **결과**: `tsc --noEmit`(frontend·backend) 통과, `npx vite build` 통과(599.20kB 청크 경고는 기존과
  동일, 무관). 🔵 **실기동은 사람이 한다**(`.harness/done.md` §1, 2026-09-03 정책) — 이번 요청은
  이전 위치정보 작업과 달리 실기동을 명시적으로 요구하지 않아 dev 서버를 띄우지 않았다. "한 줄에
  담기" 요구는 기존 flex-wrap 필터 바(`gap:1.2rem`)에 텍스트 입력을 `flex:'1.6 1 220px'`로 추가하는
  방식으로 구현 — `.main-wrapper .container`가 데스크톱(≥769px)에서 `min-width:1200px`을 보장하는
  기존 CSS(index.css:337-338, 2026-09-09) 덕에 위치(320)+검색어(220)+구분(180)+버튼(~140)+gap
  (~58) ≈ 918px가 1200px 콘텐츠 폭 안에 들어가 줄바꿈 없이 한 줄에 표시될 것으로 계산했지만,
  **실제 브라우저에서 시각 확인은 아직 안 함** — 사람 확인 필요.
- **편차**: 없음.
- **다음 에이전트가 알아야 할 것**: `/facility` 페이지에서 (1) 다양한 폭(360px·768px·1024px·
...
## 2026-09-09 | wt186 [Claude:Opus] 장례식장 정본을 공공 API로 확정 — `01-02` §2.2 + `systems.md`·`security.md`

- **근거 스펙**: 사람 확정(2026-09-09) — *"장례식장은 지금 API 통해서 작업하는 게 베스트인 것으로! 정한 거야!"* · 이름 정책·삭제 정책도 사람 확정. 관련 정본 `01-02` §2.1·§2.2 · `systems.md` §2·§3 · `00-05`(`Facility`)
- **건드린 파일**: `docs/01_장사시설_매칭/01-02_공공데이터_및_API_상업적_이용_법률_검토서.md` · `.harness/systems.md` · `.harness/security.md` · `.harness/memory/context.md` (🔴 `eobomDev/` 무변경 — Opus 세션. 구현은 Sonnet 프롬프트로 넘김)
- **결과**: 문서가 *"e하늘 장례식장 현황 OpenAPI ❌ 배제"* 한 줄만 갖고 있어 **다음 세션이 이 작업을 되돌릴 수 있는 상태**였다. 그 판정을 닫았다.
  ① **`01-02` §2.2에 확정 블록** — 승인된 것은 **보건복지부 보건·복지현황 계열 `ODMS_DATA_04_1`**(심의·승인·개발계정·활용기간 2026-08-07~2028-08-07·일일 10,000·XML 전용·기준일 2026-04-01·`totalCount` **1,080**). 🔴 **e하늘 배제 판정이 뒤집힌 게 아니라 요건이 다른 문을 찾은 것**임을 명시. 🔵 **Layer 2와 충돌 없음** — 이 API는 **위경도를 주지 않고**, 좌표는 주소 지오코딩(트랙 B 범위)으로 채운다. `geolocation` 거리 정렬(트랙 A)은 여전히 신고 대상이며 `00-22` B-1에 그대로 남는다.
  ② **`systems.md` §2** — 45건 캡 한 줄에 **실측을 붙였다**: 경기 48(실제 183)·서울 47(61)·전남 46(124)·경북 45(121), 캡 아래였던 **세종 6건만 정확**, **커버리지 601/1,080 = 55.6%**. **카카오는 정본 → 보강으로 강등**(좌표·`kakaoPlaceId`·표시용 통용명). 🔵 지오코딩은 건당 조회라 45캡이 없다.
  ③ **`systems.md` §3** — 표에 신규 행(✅ 정본) 추가 + e하늘 행에 *"다른 데이터셋"* 명시. 플레이스홀더 경고를 **`price`·`religion`·`rating` 3개로 축소**(`phone`·`location`·`tags`·`amenities`는 공공 실데이터로 대체됨). 🔴 구현 함정 3개 기록 — **chunk 경계 한글 깨짐**(`Buffer.concat` 필수, 실제로 겪음) · **페이지당 500건 상한** · **고유 관리번호 없음(이름 26건 충돌 → 매칭은 전화 > sourceRef > 이름)**.
  ④ **`security.md` §3** — 공공데이터포털 인증키 행 추가. 🔴 **값은 적지 않았다.** 백엔드 전용(`VITE_` 금지) · 09-09 평문 노출 이력 · **사람이 재발급하지 않기로 확정** · 만료 2028-08-07.
  검증: `bash .harness/tools/harness-doctor.sh` 139항목 중 3건(전부 기존 결함). `context.md` **3009B**/3072B. 🔴 **빌드 없음 — 코드 무변경 세션.**
- **편차**: **없음.** 🔵 사람 결정 2건이 내 권고와 달라 그대로 반영했다 — ① 표시 이름을 **공공명이 아니라 카카오 통용명**으로(내 A안은 반대였다) → 컬럼 역할이 뒤집혀 `publicName`(공공)+`name`(표시) 구조가 됐다. ② 미매칭 40건을 **soft delete가 아니라 삭제**(내 권고는 보존이었다) → 참조 4건이 전부 API에 존재함을 실측으로 확인해 **삭제 대상에 리뷰·클레임·리드·제휴가 0건**임을 근거로 수용했고, 그래도 **런타임 참조 가드**를 프롬프트에 넣었다.
- **다음 에이전트가 알아야 할 것**:
  🔴 **`[Claude:Sonnet]` 재적재 작업이 대기 중이다**(프롬프트는 사람이 갖고 있다). 요지: 스키마 4컬럼(`publicName`·`source`·`sourceRef`·`syncedAt`) → 3페이지 수집 → 전화 우선 2단 매칭 **id 유지 upsert** → 미매칭 API 건 신규 → **미매칭 DB 40건 삭제(가드 후)** → 지오코딩. **`--dry-run` 먼저.**
  🔴 **`schema.prisma:113`의 "id 접두사로 출처 구분" 주석이 이 작업으로 거짓이 된다** — `kakao_` 접두사 행의 `source`가 `'mohw'`가 된다. 주석 수정이 작업에 포함돼 있다.
  🔴 **삭제 `where`에 `type='장례식장'`이 반드시 들어가야 한다.** 빠지면 봉안시설·자연장지 CSV 적재분 951건이 통째로 날아간다. `backup-db.ps1` + 파일 생성 확인이 선행이다.
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: components/farewell, mypage, lib/modeNav 도메인별 4개 컴포넌트군 이동 및 13곳 import 경로 동기화 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: ObituaryPage buildPreviewData 내 memorialSlug null 고정으로 조문객 미리보기 모달 내 추모관 영역 비노출 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (07-03 §6.4 전수 부합: ObituaryView 프레젠테이션 분리, obituaryController 본인 방문 조회수 방어, 460px 90dvh 미리보기 모달 구현 및 tsc/build 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (07-03 §6.4 신설 부합: 3열 레이아웃 기각 근거 및 460px 90dvh 모달·ObituaryView 추출·조회수 가드 스펙 정립 확인, 00-38 연계 정합) -->
...
<!-- Gemini 판정: ✅통과 (00-38 §8.1-2·07-02 §2-1 부합: careGuideTasks deadlineShort 23건 전수 배선 및 CareGuidePage 모바일 칩 필터·행 UI 적응형 분리 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (07-03 §7-1 신설 정본화 완료: sms: 프로토콜 핸들러 특성 반영 모바일 전용(≤768px) 스펙화 및 ObituaryPage 구현 정합 확인) -->
...
<!-- Gemini 판정: ✅통과 (00-38 실측 백필 및 07-04 §8 #6·00-19 제4조 ⑨ 개인정보처리방침 정합 완료, 07-02 §2-1 deadlineShort 23건 명세화 확인, harness-doctor 통과) -->
...
<!-- Gemini 판정: ✅통과 (00-09 §6.3 규칙1 부합: .section-title 프리미티브 신설, 22px+ 제목 9곳 명조 통일 및 라벨/본문 4곳 명조 제거 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 피드백 부합: .container min-width 1200px 상향 및 수정 칩 104px 축소·패딩 정제 확인, /facility 실기동 검증 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 시안 선택 부합: 우상단 40px 아이콘 칩 및 호버 확장 구현, .main-wrapper 뷰포트 높이 캡으로 가로 스크롤바 노출 버그 수정 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 섹션 래퍼 justifyContent center 정렬, 수정 버튼 세이지톤(#E3E8E1) 개선 및 min-width:0 플렉스 전파 차단 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 카드 섹션 432px 확대, flex-wrap 전환을 통한 컬럼 잉여 여백 버그 해소 및 수정 버튼 시인성 개선 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 카드 폭 80% 확대 및 managePanel 내부 폰트 --fs-caption 축소, 버튼 패딩 조정 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 카톡 카드 왼쪽 정렬 margin:0 및 섹션 폭 360px 축소, 섹션 간격 조정 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 시안 선택 부합: previewCard 배경색 #DCE3E8 적용 및 두 섹션 maxWidth 420px 축소 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 카드 너비 67% 축소에 맞춘 외부 래퍼 패딩 및 수정 버튼 폭 67% 동기화 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 요청 부합: obituary_card.png 실측 기준 1:1 이미지 비율·크림색 배경·버튼 및 출처 행 추가 정합 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 레이아웃 정정 지시 부합: ObituaryPage 관리 모드 auto-grid 기반 좌측 미리보기/수정 + 우측 공유 패널 2열 배치 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: ObituaryPage 관리 모드 미리보기 우선 노출 및 수정 폼 모달 분리, .container 최소폭 1024px 배선 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (00-38 §4.5-4 Phase 1.7 전수 부합: 18개 파일 fontSize 구간 규칙에 따른 시맨틱 토큰 치환 완료, tsc/build 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (00-38 §4.5-2·§4.5-3 Phase 1.6 전수 부합: 14개 파일 fontSize 하드코딩 토큰 치환 및 padding/margin 타입 토큰 정정 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 확정 시안 A 부합: FooterMobile 아코디언 컴포넌트 신설 및 모바일 분기 배선 확인, tsc/build 통과) -->
...
<!-- Gemini 판정: ✅통과 (핸드오프 지시 부합: eobom/ -> eobomDev/ 이동 및 render.yaml/gitignore/README 내부 경로 일괄 갱신 확인, git 추적 정상) -->
...
<!-- Gemini 판정: ✅통과 (00-38 §8.1-1·06-05 §4.3 부합: FarewellMessagePage 데스크톱/모바일 뷰 분리, 고지문구 복원 및 tsc/build 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (00-38 §4·§5 Phase 1 전수 부합: useIsMobile 신설, 360px 뷰포트 기준 CSS 토큰 정합, HomePage ref 우회 적정 및 tsc/build 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (사용자 피드백 부합: FarewellMessageCard 수정 시 mediaMime 기반 탭 자동 선택 및 모달 사이드노트 flex stretch 버그 해소 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 전달 고지 배너 배경색 흰색(#FFFFFF) 및 테두리 border-color 적용 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 전달 고지 배너 경고 아이콘을 Info로 교체 및 secondary 배경·텍스트 톤 중립화 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: reports 시안 B 기준 farewell-board-shell 마크업/스타일 포팅 및 tsc/build 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (사용자 UI 지시 부합: 편지 행 maxWidth 680px 축소, 내부 padding 1.25rem 확대 및 날짜·액션 버튼 정렬 조정 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: 유족메시지 상세 영역에 md-detail 타이포그래피 및 테두리·색상 토큰 적용 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 시안 선택 부합: FarewellMessagePage 좌측 수신인 사이드바 + 우측 상세 편지함 2열 레이아웃 전환 및 tsc 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (사용자 피드백 4건 전수 부합: 수신인 카드 16px 패딩·새 편지 탭 닫기 버튼·입력란 여백 조정 및 tsc 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (사용자 시안 지시 부합: FarewellMessagePage 수신인별 카드 그리드 2열 및 새 편지 쓰기 3탭 구조 구현, tsc/build 통과 확인) -->
...
<!-- Gemini 판정: 🔄스펙갱신 (07-04 §8-9 id23 이동 반영 확인 / id12 conditional 렌더 로직 미연계에 따른 카테고리 헤더 표시 스펙 검토 필요) -->
...
<!-- Gemini 판정: ✅통과 (사용자 지시 부합: CareGuidePage 최상단 배너 상하 리본 띠 적용 및 버튼 flex 한 줄 배치 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 시안 E 선택 부합: CareGuidePage 최상단 배너 미니멀 아웃라인 스타일 적용 및 tsc 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (00-23 §8.7 및 사용자 지시 부합: CareGuidePage 상속포기 배너 텍스트 색상 중립화 및 tsc 통과 확인) -->
...
<!-- Gemini 판정: ✅통과 (사용자 요청 부합: CareGuidePage month3 구간 버튼 살구색 배경 제거 및 border-color 유지 확인, tsc 통과) -->
...
<!-- Gemini 판정: ✅통과 (사용자 요청 부합: EndingNotePage 목차 박스 font-size 0.88rem/0.78rem 조정 및 tsc 통과 확인) -->
