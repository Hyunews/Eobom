...
- `walkthrough.md`에 항목을 추가하려고 Edit를 호출했더니 "파일이 마지막으로 읽은 뒤 디스크에서
  바뀌었다"는 알림과 함께 편집은 그대로 적용됐다 — 동시에 Opus 세션이 `07-03` §6.4를 신설하며
  같은 파일 끝에 자기 항목을 붙이고 있었다. 결과: 내 항목과 Opus 항목이 **둘 다 `wt184`** 로
  번호가 겹쳤다(내 항목이 먼저 삽입된 지점 바로 뒤에 Opus 항목이 붙어 순서도 뒤집힘 — Opus가
  스펙을 먼저 쓰고 내가 그걸 구현했는데 파일상으론 내 항목이 위에 왔다).
  → **회피**: 번호를 `wt185`로 정정하고, Opus 항목 뒤로 옮겨 시간 순서를 맞췄다. 항목 안에
  "원래 wt184로 잘못 번호 매겨졌다가 정정했다"는 한 줄을 남겨 다음 세션이 번호 불일치를
  보고 헷갈리지 않게 했다.
...
## 2026-09-18 [Claude:Sonnet] 00-39 care-guide 재구현 — 삽질/메모

- **사이드바 삭제 전 실제 위험 발견**: 처음엔 00-39 §6-3(사이드바→헤더 드롭다운)을 곧이곧대로
  구현하려 했는데, `Header.tsx`의 `<nav className="header-nav">`가 `{currentUser && (...)}`로
  로그인 사용자에게만 노출되고 있었다(2026-08 개발자 확정 사항). 사이드바(로그인 무관 항상
  렌더)를 없애면 비로그인 사용자는 `/care-guide`(로그인 없이 열람 가능한 화면인데도) 포함
  모든 페이지에서 내비게이션 수단이 0이 된다 — 00-39 명세서엔 이 충돌 언급이 없었다.
  코드를 먼저 다 짜고 나서 발견했으면 되돌리기 번거로웠을 것 → **회피**: 전역 공용 컴포넌트
  (Header/Sidebar처럼 모든 페이지가 의존하는 껍데기)를 손대기 전에는 그 컴포넌트의 기존 조건부
  렌더 로직을 먼저 읽고, 스펙이 명시하지 않은 충돌 지점을 찾아 사람에게 먼저 물었다
  (AskUserQuestion) — "헤더 드롭다운 항상 노출"로 확정.
- **모달 3줄 캡(§6.4 규칙10) vs 기존 CTA 버튼**: 기존 목록 행에 있던 "장사시설 찾기 →"·
  "전문가 상담 →"·"정부24 바로가기" 3종 버튼(6개 항목)을 어디로 옮길지 애매했다 — 모달은
  제목·기한·근거 3줄로 못박혀 있어(§6.4-10) 4번째 줄로 못 넣고, §6-1(목록은 제목+기한만,
  웹·모바일 공통)이라 행에도 못 둔다. 헤더 드롭다운(`BEREAVED_MENU`)이 facility·counseling·
  digital-estate를 이미 항상 노출하므로 중복 진입점으로 보고 그냥 삭제 — walkthrough 편차로 기록.
- **정부 사이트 URL을 함부로 안 넣음**: `e하늘 장사정보시스템`(id4) 실제 URL을 정확히 확인하지
  못해 `legalLink.ts`에 href 없이 라벨만 뒀다(`정부24`·`안심상속`은 기존 JSON의 `externalUrl:
  "https://www.gov.kr"`을 그대로 재사용해 확인된 값만 씀). 근거 없는 정부 URL을 실제 사용자
  화면에 박아 넣는 게 더 위험하다고 판단 — 다음 사람이 실측 후 채울 것.
- **walkthrough.md·claude_tasks.md 둘 다 "최신 항목이 위로" 라고 헤더에 적혀 있지만 실제 최근
  항목(09-16~09-17)은 파일 맨 끝에 쌓여 있었다** — 어느 시점부터 관행이 바뀐 것으로 보임.
