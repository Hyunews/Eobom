...
import { ScrollText, FileCheck, LogIn, Printer, Copy, Download, CheckCircle2, Circle, AlertTriangle } from 'lucide-react';
...
export const EndingNotePage: React.FC<EndingNotePageProps> = ({ currentUser, onOpenLogin, setActiveTab }) => {
  ...
  // ⑨ 자필증서 유언장 필사용 초안 — 06-05 §4.2 정정(08-26)으로 음성 입력은 유족 메시지
  // 보관함으로 전부 이관됐다. 여기는 타이핑 전용 도구로 남는다. 서버 저장 없음, 브라우저
  // 임시 저장(localStorage 등)도 하지 않음.
  ...
  return (
    <div className="container" style={{ position: 'relative' }}>
      ...
      {/* ⑨ 자필증서 유언장 필사용 초안 — 06-05 §4.2 정정으로 타이핑 전용 도구로 남는다.
      ...
      <div style={{ marginTop: '1.5rem', backgroundColor: 'var(--card-bg)', padding: '1.5rem', borderRadius: 'var(--border-radius)', boxShadow: 'var(--box-shadow)', filter: !currentUser ? 'blur(3px)' : 'none' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
          직접 입력한 내용을 자필증서 유언장을 손으로 옮겨 쓸 때 참고하는 초안으로 씁니다. 이 화면에서 만든 글 자체는 유언장이 아닙니다.
        </p>
