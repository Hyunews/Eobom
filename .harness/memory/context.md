...
✅ §4.5-4-2 ㉮(사후연결) 구현완료(wt137·138: 최종 UX=버튼 하나)—Opus확인 시 문서 정정 필요.
...
✅ `06-05` D-1~D-11+UI다듬기+부고장·추모관분리+사후연결+종료후재작성버그(wt121~139, DB
마이그레이션 포함) 전부 구현·빌드통과—상세 `walkthrough.md`. 🔴 #24 보류(`backlog.md`⑪).
🟡 미검증(dev한계): `--confirm`·운영버킷2단계. ▶ 다음=Phase C(§3.3)/backlog.
...
마지막  : 2026-09-07 — 부고장·추모관 분리+사후연결+종료후재작성(wt135·137~139) / wt112·114·115 대기
```
