...
✅ §4.5-4-2 ㉮(사후연결) 구현완료(wt137)—Opus 확인 시 "사후연결 수단 없음" 문장 갱신 필요.
...
✅ `06-05` D-1~D-11+UI다듬기+부고장·추모관분리+사후연결(wt121~137, DB마이그레이션 포함)
전부 구현·빌드통과—상세 `walkthrough.md`. 🔴 #24 보류(`backlog.md`⑪). 🟡 미검증(dev한계):
...
마지막  : 2026-09-07 — 부고장·추모관 분리+사후연결(wt135·137, DB마이그레이션) / wt112·114·115 대기
```
