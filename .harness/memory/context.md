...
**1~3. ✅ 08-26 완료**(walkthrough 74) — `06-05` Phase A(보관함 화면·슬라이드 분리) · 초대 흐름
3건(`00-27` §9.1-4~9.1-4-3: 카카오SDK 폴백사다리·수락완료 CTA·성함대조) · 발신번호(§11-2 닫힘).
✅ **가족 연결은 실기기 확인까지 완료**(08-26 사장님).

**4. 🔴[사장님] STT 실구현 = Ⓐ mp3 업로드**(08-26 지시, 스펙 `06-04` §6.4-9). Ⓑ직접녹음은 완료됨.
✅ 국외이전은 블로커 아님(`00-17` §3.2 ③ — 동의 불요, `00-19` 묶음개정에 얹으면 비용 0).
🔴 **남은 건 업체 선정+유료계약 하나**(`roles.md` §4 승인). 요건 7개 §6.4-9-2 —
🔴 **#1 학습 활용 opt-out이 계약서에 없으면 탈락**(가격 볼 필요 없음).

**5. [사용자] 남은 확정** — `00-14` §11-3(사망확인 SLA)·§11-11(06 수익지점) · `00-20` 보유기간 ·
위치정보법 신고. ⏸ 11-1 알림톡은 **발신번호 사전등록**(§11.4 b)이 별건이라 아직 안 열림.

**5. [Claude:Opus] `00-06` 화면ID 신설** — SCR-016·017(`/prep`·`/bereaved`) + `SCR-002-B`(보관함).

**6. ✅[Sonnet] 홈 박스③④ 높이 통일**(08-26, walkthrough 75) — 200px→`entry-box-primary`
300px로, `BoxHeader size="lg"`로 ①②와 맞춤.

**7. [Gemini] 게이트 대기 10건**(walkthrough 70~75). 실기동 전부 미검증.
