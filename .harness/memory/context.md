...
**1. ✅[Sonnet] `06-05` Phase B 완료**(08-26, walkthrough 77) — `EndingNote`·`FarewellMessage`
모델+API 배선, STT(Ⓐ업로드·Ⓑ녹음)를 엔딩노트 ⑨에서 유족 메시지 보관함으로 이관. 실측: API
작성→조회→수정 전 구간, DB `bodyEnc` 암호화 확인, 브라우저 렌더 확인(Claude in Chrome). 🔴
**`CLOVA_STT_ENABLED`는 여전히 false** — 플래그 켠 m4a 실업로드→저장은 사람이 직접 확인 필요.
...
**4. [Gemini] 게이트 대기 12건**(walkthrough 70~77). 🔴 STT 발화 정확도·나머지 실기동 미검증.

✅ **08-26 완료**(walkthrough 74~77) — `06-05` Phase A·B · 초대 흐름 3건 · 발신번호 · 홈 박스③④ ·
STT Ⓐ 배선+보관함 이관. **가족 연결 실기기 확인 완료**(사장님).
...
마지막  : 2026-08-25 [Opus] 06-05 도메인분리 + 하네스 결함7건 수정 / 2026-08-26 [Sonnet] walkthrough 70~77
```
