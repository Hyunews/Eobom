# security.md — 개인정보·시크릿·발행 게이트

> **언제 읽나**: 개인정보(유족·고인·상담자) 관련 코드/문서를 건드릴 때 / 시크릿·API 키를 다룰 때 / 외부에 무언가 공개·배포할 때
>
> 이어봄은 **사별·상속·유언**을 다룬다. 일반 웹서비스보다 민감도가 한 단계 높다는 전제로 작업한다.

---

## 1. 절대 금지 (예외 없음)

- **실제 개인정보를 저장소에 커밋하지 않는다.** 유족·고인·상담 신청자의 이름·연락처·주소·가족관계는 어떤 파일에도 원문으로 남기지 않는다. `docs/`·`reports/`·로그·테스트 픽스처 전부 해당.
- **시크릿을 코드·문서에 하드코딩하지 않는다.** OAuth 클라이언트 시크릿, JWT 서명키, DB 접속 문자열, 카카오 REST 키는 `.env`에만. `.env`는 `.gitignore`에 이미 포함되어 있다.
- **에러 메시지·로그에 개인정보를 흘리지 않는다.** 디버깅용 `console.log(user)` 같은 전체 객체 덤프 금지 — 필요하면 `user.id`만.

## 2. 예시·테스트 데이터는 반드시 가짜로

문서나 테스트에 사람 정보가 필요하면 명백한 더미를 쓴다.

| 쓰지 말 것 | 쓸 것 |
|---|---|
| 실제 지인 이름·전화번호 | `홍길동`, `010-0000-0000` |
| 실제 이메일 | `test@example.com` |
| 실제 주소 | `서울시 중구 세종대로 110` (공공청사) |

시설(장례식장·봉안시설) 정보는 공개된 공공데이터이므로 실제 값 사용 가능.

## 3. 시크릿 목록과 위치 (값은 절대 여기 안 적는다)

| 시크릿 | 위치 | 비고 |
|---|---|---|
| 카카오/네이버/구글 OAuth 클라이언트 시크릿 | `eobomDev/backend/.env` | 3사 소셜 로그인 |
| JWT 서명키 | `eobomDev/backend/.env` | |
| `DATABASE_URL`·`DIRECT_URL` | `eobomDev/backend/.env` · Render 대시보드 | 로컬=Docker / 배포=Supabase(`systems.md` §4) |
| `SETTLEMENT_ENCRYPTION_KEY` | `eobomDev/backend/.env` · Render | 정산·조의금 계좌 암호화 |
| `ENDING_NOTE_ENCRYPTION_KEY` | `eobomDev/backend/.env` · Render | 06 엔딩노트·유족 메시지·R2 음성 선암호화(`00-11` §5.4-4) |
| `HASH_INDEX_KEY` | `eobomDev/backend/.env` · Render | 가족지정 연락처 중복방지 해시(`00-33` §4.2). 🔴 정산 키와 **공유 금지** — 그 결합이 TS-001 원인 |
| `R2_ACCESS_KEY_ID_*`·`R2_SECRET_ACCESS_KEY_*` | `eobomDev/backend/.env` · Render | 버킷별 3쌍(`00-11` §5.4-6-1). 🔴 로컬은 **`-dev` 버킷 토큰**, 운영 토큰은 Render에만(§5.4-6-3) |
| `VITE_KAKAO_MAP_KEY` | `eobomDev/frontend/.env` | **프론트 번들에 노출됨** — 도메인 제한 필수 |
| 🆕 **공공데이터포털 인증키 + End Point** | `eobomDev/backend/.env` | 보건복지부 장례식장 현황(`ODMS_DATA_04_1`, `systems.md` §3). **백엔드 배치 전용 — 프론트로 내보내지 않는다**(`VITE_` 접두사 금지). 🟡 2026-09-09 대화 중 평문 노출된 적 있음 — **사람이 재발급하지 않기로 확정**. 활용기간 만료 **2028-08-07** |

🔴 **암호화·해시 키 3종은 로컬과 운영이 서로 다른 값이다**(2026-08-27 교체). 같으면 로컬 `.env`
유출이 곧 운영 데이터 유출이고, 운영 덤프를 로컬에서 복호화할 수 없는 것이 §1(운영 개인정보를
로컬로 가져오지 않는다)을 기술적으로 강제한다. 🔵 로컬 DB와 운영 DB가 별개라 값이 달라도 문제없다.

🔴 **반대로 한 환경 *안에서* 바뀌면 안 된다.** 암호문은 복호화 불가가 되고, `HASH_INDEX_KEY`는
중복방지가 **조용히** 깨진다(TS-001). 바꿔야 하면 `00-33` §4.3·§5 교체 절차 — 백업 → 전 행
재암호화·재해시(단일 트랜잭션) → dry-run 값 비교. `db-safety.md` 전 과정이 걸린다.

> `VITE_` 접두사 변수는 빌드 결과물에 그대로 박힌다. 비밀이 아니라 "공개되지만 도메인으로 잠근 키"로 취급할 것.

## 4. 위치정보 (위치정보법)

`docs/01_장사시설_매칭/01-02_공공데이터_및_API_상업적_이용_법률_검토서.md`가 정본. 요약:

- **트랙 A — 실시간 GPS(`navigator.geolocation`)**: 위치정보사업자 **신고 대상**. 현재 사용자는 사업자 신고 전이므로 실서비스 전환 시 반드시 제거 또는 신고 완료 후 사용.
- **트랙 B — 시/도·시/군/구 수동 선택**: 신고 불필요. 실서비스 기본 경로로 삼는다.
- 현재 로컬 개발 단계에서는 A/B가 공존 중. **프로덕션 배포 전 A 제거 여부를 반드시 재확인**한다.

## 5. 발행 게이트 — 밖으로 나가는 것

아래는 실행 전 **사용자 승인 필수**. 되돌리기 어렵거나 외부에 노출되기 때문.

- `git push`, 공개 저장소 전환, 배포(Vercel 등)
- `reports/` 산출물을 사내·대외로 공유
- 외부 API에 실데이터 전송, 새 외부 서비스 연동

> 🔴 **2026-09-21 정정 — 저장소는 현재 `PUBLIC`이다**(`github.com/Hyunews/Eobom`). 아래 *"private이라는 것 하나"* 라던 방어선은 **이미 없다** → §6-1. public 전환 시 `docs/` 전체를 개인정보·사업 기밀 관점에서 재검토해야 한다(`reports/`는 2026-08-20부터 커밋 제외 → `roles.md` §1-1).
>
> 🔴 **public 전환·협업자 추가·외주 투입은 "지금 파일을 지우면 되는" 문제가 아니다.** git 히스토리는 파일을 삭제해도 과거 커밋에 남으므로, **전환 시점이 아니라 전환을 검토하는 시점에** 히스토리 전체를 대상으로 판단해야 한다. 지금까지의 실질적 방어선은 **저장소가 private이라는 것 하나**다.

## 6. 데이터 보호 (운영)

- 🔴 **DB 쓰기 게이트는 `db-safety.md`가 정본이다**(2026-08-27 분리 — 트리거가 명령 단위로
  넓어지면서 이 문서의 주제를 벗어났다). **DB에 쓰기 전 백업이며 스키마 변경만이 아니다.**
  유실 2회(08-05 마이그레이션 · 08-27 정리 스크립트).
- 🔴 **백업 파일에는 개인정보가 들어간다. 절대 커밋하지 않는다**(§1).
  🔴🔴 **2026-09-21 — 이 규칙이 실제로 깨졌다.** 아래 §6-1.
- **엔딩노트**: 사후 전달 콘텐츠는 저장 시 암호화(AES-256) 전제로 설계한다. 평문 저장 구현을
  임시로라도 만들지 않는다 — 임시가 그대로 남는다.

### 6-1. 🔴🔴 2026-09-21 사고 — **DB 덤프 2개가 공개 저장소에 올라가 있다**

| 무엇 | 사실 |
| :--- | :--- |
| 파일 | `eobomDev/backend/prisma/backups/local-20260907_115023.dump`(189KB) · `local-20260921_132135.dump`(252KB) |
| 커밋 | `07d10c3` · `e6be50b` — **둘 다 `origin/main`에 push됨** |
| 저장소 | 🔴 **`github.com/Hyunews/Eobom` = PUBLIC** (아래 §5 문구는 *private* 이라고 적혀 있었다 — 낡은 기술) |
| 들어 있는 것 | 로컬 개발 DB **전체**(`-Fc`). `User.email`·`refreshToken` · `Lead.applicantPhone`·`ConsultRequest.applicantPhone`(🔴 **평문 연락처**) · `FamilyDesignation.phoneEnc`(암호문) · `Admin`·`Partner`·`Expert.passwordHash`(bcrypt) 등 |

🔴 **원인은 규칙이 아니라 경로였다.** `.gitignore`는 `eobomDev/backend/backups/`·`/backups/`를
막고 있었는데 `backup-db.ps1`이 실제로 떨어뜨리는 곳은 **`eobomDev/backend/prisma/backups/`** 다.
**규칙은 있었고 그물만 어긋나 있었다** — 그래서 아무 경고 없이 두 번 통과했다.

🔴 **`.gitignore`에 실제 경로를 추가했다**(2026-09-21). 재발은 막혔지만 **이미 올라간 것은
그대로 남아 있다** — 아래는 사람이 정한다.

| 순서 | 무엇 | 누가 |
| :---: | :--- | :--- |
| 1 | **저장소를 private으로** — 가장 빠른 봉쇄 (`gh repo edit Hyunews/Eobom --visibility private`) | 사람 |
| 2 | 추적 해제 + 커밋 (`git rm --cached <두 파일>`) | 사람 |
| 3 | **히스토리에서 제거**(`git filter-repo`/BFG + force push) — 🔴 파괴적. public이었으므로 **force push 후에도 GitHub에 dangling commit이 남을 수 있어** 지원팀 GC 요청까지 해야 완전하다 | 사람 결정 |
| 4 | **회전** — `User.refreshToken` 무효화(🔴 DB 쓰기 = 백업+CONFIRM) · `Admin`·`Partner`·`Expert` 비밀번호 변경 | 사람 결정 |
| 5 | 덤프에 **실제 지인·테스터의 연락처**가 들어 있는지 확인 → 있으면 고지 여부 판단 | 사람 |

##### 진행 상황 (2026-09-21)

- ✅ **BFG 재작성 + `main` force-push 완료** — `origin/main`=`c6a1e5b`, 덤프 blob **0건**. 커밋 317개 그대로, 차이는 덤프 2개 삭제뿐. 룰셋(`non_fast_forward`)은 일시 해제 후 **`active`로 복구 확인**.
- 🔴 **그래도 서버에는 남아 있다** — 옛 커밋(`e6be50b`·`07d10c3`·`fa7f847`)과 blob 2개가 **SHA로 여전히 조회됨**. `push`는 브랜치 포인터만 옮기고 객체를 지우지 않는다.
- ✅ **GitHub 지원 티켓 `#4776606`** — *"Request garbage collection of unreachable objects after history rewrite"*, 2026-09-21 14:33 접수, **열림**.
  근거 자료: First Changed Commit **`07d10c3`**(2026-09-07T02:58:14Z) · 영향 PR **0건**(`refs/pull/1/head`·`/merge` 둘 다 blob 0 확인) · LFS **미사용** · 포크 **0**.
  🔵 심사 기준이 *"자격증명 회전으로 완화되면 지원 안 함"* 이라, **제3자 실명·연락처는 회전으로 되돌릴 수 없다**는 점을 티켓에 명시했다.
- 🔴 **남은 것** — ⓐ 로컬 `gc`(이 PC에 옛 객체 잔존) ⓑ 협력자 `devhyaC` 제거(개인 저장소는 read 강등 불가 — 422) ⓒ `refreshToken` 6건·비밀번호 5건 회전 ⓓ 공개/비공개 결정.

🔵 **덤프는 개발 DB다.** 운영 데이터는 아니지만 `security.md` §1이 막는 것은 *"실제 개인정보"*
이지 *"운영 DB"* 가 아니다 — 지인·테스터의 실제 연락처면 그대로 해당한다.

---

## 7. 의심스러우면

판단이 안 서면 진행하지 말고 사용자에게 묻는다. 개인정보·법률 영역에서 에이전트의 자율 판단 범위는 **좁게** 잡는다.
