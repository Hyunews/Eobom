...
# ── 12. 절대경로 하드코딩 (살아있는 문서) ────────────────────────
# 왜 있나: 2026-09-16 경로 이동(`C:\…\Desktop\Eobom` → `D:\Eobom`)에서 `file:///c:/…`
#   절대링크 **26건이 한꺼번에 깨졌다.** 반면 상대로 적힌 것은 **하나도 안 깨졌다** —
#   훅의 `$CLAUDE_PROJECT_DIR`, `.gitignore`, `ROOT=$(dirname "$0")/../..`, `render.yaml`.
#   차이는 절대/상대 하나뿐이었다. 사람 방침(09-16): **"가능하면 상대경로."**
#
# 🔴 규칙만 두면 또 쌓인다. 26건이 몇 달간 아무에게도 안 걸린 이유가 **아무도 안 쟀기**
#   때문이다(§10이 `^eobom/`로 굳어 조용히 통과하던 것과 같은 모양). 그래서 여기서 잰다.
#
# 🔵 제외 대상과 근거:
#   · docs/작업일지_및_기록/ — 과거 로그. "그때 그 경로였다"는 기록이라 고치면 사실이 틀어진다
#   · .harness/_meta/       — 폐기된 검토안 아카이브
#   · `https://` URL        — 배포 도메인(vercel/onrender). 상대경로로 바꿀 수 없다
#   · 저장소 밖 경로        — `00-25`의 `c:\Users\…\Desktop\interface`(외부 분석 대상)처럼
#                             저장소 밖이면 상대경로로 표현할 방법이 없다. `file:///`만 잰다
echo "12. 절대경로 하드코딩 — 살아있는 문서의 file:/// 링크"
CHECKS=$((CHECKS + 1))
ABS_HITS=$(grep -rn --include='*.md' -E 'file:///[a-zA-Z]:' "$ROOT/docs" "$HARNESS" 2>/dev/null \
           | grep -v '작업일지_및_기록' \
           | grep -v '/_meta/' || true)
ABS_N=$(printf '%s' "$ABS_HITS" | grep -c . || true)
if [ "$ABS_N" -eq 0 ]; then
  ok "살아있는 문서에 file:/// 절대링크 없음"
else
  fail "file:/// 절대링크 ${ABS_N}건 — 폴더를 옮기면 그대로 깨진다. 상대경로로 바꿀 것"
  printf '%s\n' "$ABS_HITS" | head -5 | sed "s|^$ROOT/|       · |"
  gray "       같은 저장소 안이면 ../ 로 닿는다. 2026-09-16에 26건이 이 이유로 깨졌다."
fi
echo
