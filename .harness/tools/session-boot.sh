...
#
# 싣는 것 : §0-1(조건부 로드 표) · §1(작업 순서·CONFIRM·DB) · §2(소유권)
#           · §10(읽기 예산 — 2026-09-02 추가. 매 턴 반복되는 판단이라 상시로 있어야 한다)
#           🔴 **머리말은 2026-09-02부터 안 싣는다**(1,073B) — 3주체 정의와 "충돌 시 AGENTS.md가
#           이긴다"는 아래 꼬리말 한 줄로 옮겼고, 나머지(CLAUDE.md 분리 기각 근거 등)는 하네스를
#           고칠 때만 필요한 메타 서술이라 매 세션 실을 이유가 없다.
...
emit_rule_sections() {  # emit_rule_sections <표시이름> <경로>
  ...
  body="$(awk 'BEGIN{keep=0} /^## /{keep=($0 ~ /^## (0-1|1|2|10)\./)} keep' "$path")"
  ...
  printf '> 🔵 주체 태그는 `[Claude:Opus]`(기획) / `[Claude:Sonnet]`(구현) / `[Gemini]`(문서화·교차검증) 셋이고, 규칙이 충돌하면 **AGENTS.md가 이긴다**.\n'
}
