...
- 🔴 **시/도단위 검색시 45건캡**(15건×3페이지)으로 일부지역 누락. 전수수집은 시/군/구 재검색 필요.
  🔴 **09-09 실측 — 이 캡이 장례식장 데이터를 실제로 잘라놨다**: 경기 **48**(실제 183)·서울 47(61)·
  전남 46(124)·경북 45(121). 캡 아래였던 **세종 6건만 정확**했다. **커버리지 601/1,080 = 55.6%.**
  ✅ **그래서 장례식장 정본을 공공 API로 옮긴다**(§3 · `01-02` §2.2, 09-09 사람 확정).
  **카카오는 정본이 아니라 보강**이다 — 좌표·`kakaoPlaceId`·화면 표시용 통용명(`Facility.name`).
  🔵 **주소→좌표 지오코딩은 키워드 검색이 아니라 건당 조회라 45건 캡이 없다.**
...
- 🔴 **API 응답 수신 함정(09-09 실제로 겪음)**: `res.on('data', c => str += c)`로 이어붙이면
  **chunk 경계에서 한글이 깨진다**(`전북특별자치도` → `전��특별자치도`). `Buffer.concat(bufs)
  .toString('utf8')`로 받을 것. **원본 데이터 문제가 아니다.**
- ⚠️ `numOfRows=1000`을 줘도 **페이지당 500건**이 상한이다(1,080건 = 3페이지).
  `type=json`·`dataType=JSON`은 **에러 응답**이 온다 — XML 파싱 고정.
- 🔴 레코드에 **고유 관리번호가 없다**. `sourceRef`는 `시도|시군구|시설명`으로 합성하며,
  **이름만으로는 26건이 충돌**한다. 매칭 우선순위는 **전화번호 > sourceRef > 정규화 이름**이다
  (카카오=통용명 / 공공=법인 등록명이라 이름 단독은 431/601밖에 안 맞았다).
